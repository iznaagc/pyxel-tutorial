...
**ブランチ**: `task/asset-loading-support`  
**担当**: Claude Code  
**変更ファイル**: `tools/gui_editor.py`, `tools/gui_editor.py.bak_step4f`, `documents/history/works.md`

### 概要

LayerPanel に名前編集（ダブルクリック）、上下移動ボタンを追加し、レイヤー操作を完成させました。

### 変更内容

| 項目 | 内容 |
|------|------|
| バックアップ作成 | `tools/gui_editor.py` を `tools/gui_editor.py.bak_step4f` として退避 |
| 名前編集 | ダブルクリックで QInputDialog を表示し、レイヤー名を変更可能に |
| 上移動ボタン (↑) | 選択レイヤーを1つ上に移動。layers 配列をスワップ |
| 下移動ボタン (↓) | 選択レイヤーを1つ下に移動。layers 配列をスワップ |
| ボタン整理 | +/−/↑/↓/Eye の5ボタン構成に拡張。ツールチップ付き |
...
- オフスクリーンテストで以下を確認:
  - レイヤー追加/削除が動作
  - 上下移動でレイヤー順が正しく入れ替わる
  - リネーム（シミュレーション）が反映される
  - 表示/非表示トグルが動作
  - 全モード切替が正常動作

### 備考

- ドラッグ並べ替えは上下ボタンで代替（シンプルで確実）
- STEP 4 のサブタスク（4-A〜4-F）が全て完了
- 残: STEP 7（ドキュメント最終更新）
